...
#df_hh_data = pf.compile_hh_data_ARquants_IE(filepath, combinations, name_add)
#df_hh_data.to_csv(filepath_save + 'df_hh_data_ARquants_IE_BaselineROF.csv')
print('processed data for Policy A')

#%% process household-level data-
# reprocess hh level data- hifh rof policy: rof=0.89, first inf=mcasr
filepath = '../../../../../scratch/users/jskerker/AffordPaper1/Figure5/HighROF_RandomSims_IE/'
name_add = 'IE_'
# get combinations to loop through
df_combinations = pd.read_csv(filepath + 'climate_scenarios_500_2025-12-13.csv')
combinations = df_combinations.values.tolist()

#df_hh_data = pf.compile_hh_data_ARquants_IE(filepath, combinations, name_add)
#df_hh_data.to_csv(filepath_save + 'df_hh_data_ARquants_IE_HighROF.csv')
print('processed data for Policy C')

#%% process household-level data-
# reprocess hh level data- low rof policy: rof=0.05, first inf=desal
filepath = '../../../../../scratch/users/jskerker/AffordPaper1/Figure5/LowROF_RandomSims_IE/'
name_add = 'IE_'
# get combinations to loop through
df_combinations = pd.read_csv(filepath + 'climate_scenarios_500_2025-12-13.csv')
combinations = df_combinations.values.tolist()
...
#df_hh_data = pf.compile_hh_data_ARquants_IE(filepath, combinations, name_add)
#df_hh_data.to_csv(filepath_save + 'df_hh_data_ARquants_IE_LowROF.csv')
print('processed data for Policy B')

#%% reimport data- baseline policy
# baseline policy: rof=0.61, first inf=desal
# PROCESS DATA
filepath = '../../../../../scratch/users/jskerker/AffordPaper1/Figure4/data_IE/'

# 1. Import df hh data
df_hh = pd.read_csv(filepath_save + 'df_hh_data_ARquants_IE_BaselineROF.csv')
...
df_combinations = pd.read_csv(filepath + 'climate_scenarios_{}_2025-12-12.csv'.format(count))
climate_combinations = df_combinations.values.tolist()
df_policyA_desal = pf.aggregate_sows_for_policy(filepath, rof, inf_order, climate_combinations, 2020, 2071, name_add)
#print(df_policyA_desal)

# 3. Merge SOW & AR data
df_hh_policyA = df_policyA_desal.merge(df_hh, on=['real', 'dT', 'dP', 'dCV', 'demand'])
print('policy A: ', df_hh_policyA)
df_hh_policyA.to_csv(filepath_save + 'df_hh_policyA_IE_Figure5.csv', index=False)
...
# 1. Import df hh data
df_hh = pd.read_csv(filepath_save + 'df_hh_data_ARquants_IE_LowROF.csv')
...
df_combinations = pd.read_csv(filepath + 'climate_scenarios_{}_2025-12-13.csv'.format(count))
climate_combinations = df_combinations.values.tolist()
df_policyB_desal = pf.aggregate_sows_for_policy(filepath, rof, inf_order, climate_combinations, 2020, 2071, name_add)

# 4. Merge SOW & AR data
df_hh_policyB = df_policyB_desal.merge(df_hh, on=['real', 'dT', 'dP', 'dCV', 'demand'])
print('policy B: ', df_hh_policyB)
df_hh_policyB.to_csv(filepath_save + 'df_hh_policyB_IE_Figure5.csv', index=False)

#%% reimport data- high ROF policy
filepath = '../../../../../scratch/users/jskerker/AffordPaper1/Figure5/HighROF_RandomSims_IE/'

# 1. Import df hh data
df_hh = pd.read_csv(filepath_save + 'df_hh_data_ARquants_IE_HighROF.csv')
...
df_combinations = pd.read_csv(filepath + 'climate_scenarios_{}_2025-12-13.csv'.format(count))
climate_combinations = df_combinations.values.tolist()
df_policyC_mcasr = pf.aggregate_sows_for_policy(filepath, rof, inf_order, climate_combinations, 2020, 2071, name_add)

# 3. Merge SOW & AR data
df_hh_policyC = df_policyC_mcasr.merge(df_hh, on=['real', 'dT', 'dP', 'dCV', 'demand'])
print('policy C: ', df_hh_policyC)
df_hh_policyC.to_csv(filepath_save + 'df_hh_policyC_IE_Figure5.csv', index=False)
...
#ax4.set_title('Added supply cost vs unmet demand')
...
for spine in ax6.spines.values():
    spine.set_visible(False)
ax6.scatter(df_hh_policyA['percReliability'], df_hh_policyA['AR_80'], c='salmon', marker='o', s=8, label='Strategy A', alpha=0.3, zorder=3)
ax6.scatter(df_hh_policyB['percReliability'], df_hh_policyB['AR_80'], c='olivedrab', marker='o', s=8, label='Strategy B', alpha=0.2, zorder=3)
ax6.scatter(df_hh_policyC['percReliability'], df_hh_policyC['AR_80'], c='mediumturquoise',  s=8, marker='o', label='Strategy C', alpha=0.3, zorder=3)
...
custom_legend = [
    ...
    Line2D([0], [0], marker='o', color='none', linestyle='None', markerfacecolor='mediumturquoise', markersize=sz, markeredgecolor='none', label='Strategy C, ASR First')
]
...
# save figure
plt.savefig('../outputs/Figure5_Policy_comparison_IncomeEstimation_14Dec.png', dpi=300, bbox_inches='tight')
